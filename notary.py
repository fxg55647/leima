...
def _build_notarized_email(to: str, from_addr: str, original_raw: bytes, manifest: dict, gateway: str, leima_url: str = "") -> bytes:
    ...
    verify_line = f"Tarkista:      {validate_url}\n" if validate_url else ""
    body = (
        "Tämä sähköposti on notarisoitu Leimalla.\n\n"
        f"Lähettäjä:     {manifest.get('from', '')}\n"
        f"Vastaanottaja: {manifest.get('to', '')}\n"
        f"Aihe:          {manifest.get('subject', '')}\n"
        f"Päivämäärä:    {manifest.get('date', '')}\n"
        f"DKIM:          {manifest.get('dkim', '')}\n\n"
        f"SHA-256:  {manifest.get('email_sha256', '')}\n"
        f"Arweave:  {arweave_url}\n"
        f"{verify_line}"
        "\n---\nLeima — pysyvä digitaalinen notaari\n"
    )
