...
def _build_notarized_email(to: str, from_addr: str, original_raw: bytes, manifest: dict, gateway: str, leima_url: str = "") -> bytes:
    ...
    body = (
        ...
        "1. Open the Arweave URL above. You will see the notarization manifest\n"
        "   containing the SHA-256 hash, DKIM result, sender, and timestamp\n"
        "   as recorded at the moment of notarization.\n\n"
        "2. The attached original.eml is the raw email that was notarized.\n"
        "   To verify the hash matches, run:\n"
        "     sha256sum original.eml         (Linux/macOS)\n"
        "     certutil -hashfile original.eml SHA256  (Windows)\n"
        "   The result must match the SHA-256 above.\n\n"
        "3. DKIM: the sender's cryptographic signature was verified at the time\n"
        "   of notarization. The result is permanently recorded on Arweave.\n"
        "   Even if the sender's DKIM key is later rotated or removed, the\n"
        "   notarization record proves the signature was valid at this moment.\n\n"
        "The Arweave record is permanent and cannot be altered or deleted.\n"
        "Verification does not require trusting Leima — the Arweave record\n"
        "and the attached .eml file are sufficient.\n"
