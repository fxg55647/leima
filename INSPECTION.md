...
The same principle extends to digital environments. The Witness App ([WITNESS_APP.md](WITNESS_APP.md)) applies an equivalent protocol to web sessions: a purpose-built Android browser captures screenshots, SSL certificate details, and a browsing trail at the moment the user chooses, signs the package with a hardware-backed key, and submits it to Leima for permanent stamping. A bank balance, a supplier's website, a portal document — anything visible in a browser can be captured under the same observation discipline as a physical inspection. The observer changes; the trust model does not.
