...
2. [How the proof works](#2-how-the-proof-works)
3. [Use cases](#3-use-cases) — see also [USECASES.md](USECASES.md) for extended examples
4. [Why this matters](#4-why-this-matters)
5. [A public good](#5-a-public-good)
...
The ethical questions behind these design choices — when it is right to deploy imperfect AI, what proportional safety actually means, and where categorical limits apply regardless of economic calculation — are explored in [The Cost of Safety?](COST-OF-SAFETY.md) (also available [in Finnish](TURVALLISUUDEN-HINTA.md)).
...
See [TREAD.md](TREAD.md) for a full description of the protocol, the current implementation, the longer-term vision, real-world incidents where deployment transparency would have helped (PHP git compromise, Picreel), and how TREAD relates to existing approaches such as Sigstore and Meta Code Verify. TREAD.md also discusses the relationship between statistical AI evaluation and deterministic zero-knowledge proofs, the limits of the approach including prompt injection risk, and mitigations. For a detailed component-level threat model and vulnerability analysis, see [SECURITY_MODEL.md](SECURITY_MODEL.md).
